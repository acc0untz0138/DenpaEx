...
class CrashState extends flixel.FlxState {
	...
	override function create() {
		...
		var text:FlxText = new FlxText(5, 5, 450 * 1.75, (log != null ? log : "NO CRASH DETECTED"),
			24).setFormat(null, 20, 0xffff0000, LEFT, OUTLINE, flixel.util.FlxColor.BLACK);
		text.alpha = 0;
		add(text);
	...
	inline function openURL(url:String) #if linux Sys.command('/usr/bin/xdg-open', [url]); #else FlxG.openURL(url); #end
}
