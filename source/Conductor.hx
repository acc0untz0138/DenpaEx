...
class Conductor {
	...
	public static function judgeNote(note:Note, diff:Float = 0, ?bot:Bool = false) {
		...
		final timingWindows:Array<Int> = [
			ClientPrefs.settings.get("perfectWindow"),
			ClientPrefs.settings.get("sickWindow"),
			ClientPrefs.settings.get("goodWindow"),
			ClientPrefs.settings.get("badWindow"),
			ClientPrefs.settings.get("shitWindow")
		];
