...
class GameOverSubstate extends MusicBeatSubstate {
	...
	public function new(x:Float, y:Float, camX:Float, camY:Float) {
		super();
		if (Paths.characterMap.exists(PlayState.characterVersion)
			&& Paths.characterMap.get(PlayState.characterVersion) != Paths.currentModDirectory
			&& PlayState.characterVersion != 'bf') {
			Paths.setModsDirectoryFromType(CHARACTER, PlayState.characterVersion, false);
			usingSepModFolder = true;
		} else if (Paths.characterMap.exists(PlayState.SONG.assets.player1)
			&& Paths.characterMap.get(PlayState.SONG.assets.player1) != Paths.currentModDirectory) {
		...
		boyfriend.animation.finishCallback = name -> {
			if (playingDeathSound || name != 'firstDeath')
				return;
