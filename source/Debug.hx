...
class Debug extends FlxBasic {
	...
	public function onUpdate():Void {
		if (isDebug_)
			FlxG.mouse.visible = debugMode;
		if (!isDebug_ || mainGroupLen < 0 || !debugMode || cease)
			return;
		...
		if (FlxG.mouse.justPressed) {
			initialXY = [
				getObjectFromClass(targetClass, targetObj + ".x"),
				getObjectFromClass(targetClass, targetObj + ".y")
			];
		...
		if (FlxG.mouse.justReleased)
			trace("current target x: " + getObjectFromClass(targetClass, targetObj + ".x") + " | current target y: "
				+ getObjectFromClass(targetClass, targetObj + ".y"));
