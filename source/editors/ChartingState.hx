...
class ChartingState extends MusicBeatState {
	...
	final zoomList:Array<Float> =
		#if !html5
		[
			0.0625, 0.125, 0.25, 0.5, 1, 2, 3, 4, 6, 8, 12, 16, 20, 24, 32, 48, 64, 96, 128, 192, 256, 384, 512, 768, 1024, 2048, 4096, 8192
		];
	...
	override function create() {
		...
		if (PlayState.SONG.assets.arrowSkin != null && PlayState.SONG.assets.arrowSkin.length > 1)
		...
		final text = "LEFT CLICK - Add/Remove Note
		...
		\nLEFT / RIGHT BRACKET - Change Playback Rate
	...
	function addOptionsUI():Void {
		...
		p4ModChartDropDown = new FlxUIDropDownMenuCustom(stageDropDown.x, player2DropDown.y + 40,
			FlxUIDropDownMenuCustom.makeStrIdLabelArray(modcharts, true), function(character:String) {
	...
	function addSectionUI():Void {
		...
		var clearLeftSectionButton:FlxButton = new FlxButton(340, clearSectionButton.y - 50, "Clear Left", function() {
		...
		var clearRightSectionButton:FlxButton = new FlxButton(340, clearSectionButton.y - 25, "Clear Right", function() {
			if (_song.notes[curSection].sectionNotes == null)
				return;
			var removeThese = [];
			for (noteIndex in 0..._song.notes[curSection].sectionNotes.length) {
	...
	function addNoteUI():Void {
		...
		var directories:Array<String> = [
			Paths.mods('scripts/notetypes/'),
			Paths.mods(Paths.currentModDirectory + '/scripts/notetypes/')
		];
		for (i in 0...directories.length) {
			var directory:String = directories[i];
			if (FileSystem.exists(directory)) {
		...
		var spamButton:FlxButton = new FlxButton(noteTypeDropDown.x, noteTypeDropDown.y + 40, "Add Notes", function() {
			if (curSelectedNote != null) {
				for (i in 0...Std.int(spamLength)) {
					addNote(curSelectedNote[0] + (15000 / Conductor.bpm) / spamCloseness, curSelectedNote[1], curSelectedNote[2], false);
				}
				updateGrid(false);
				updateNoteUI();
			}
	...
	function addEventsUI():Void {
		...
		var directories:Array<String> = [
			Paths.getPreloadPath('scripts/events/'),
			Paths.mods('scripts/events/'),
			Paths.mods(Paths.currentModDirectory + '/scripts/events/')
		];
		for (i in 0...directories.length) {
			var directory:String = directories[i];
			if (FileSystem.exists(directory)) {
	...
	function updateBpmText() {
		bpmTxt.text = "Pos: "
			...
			+ curStep
			+ "\n\nQuant: "
			+ quantOverlayData[curQuantOverlay][3] 
			+ "\n\nDifficulty: " 
			+ difficultyString 
			+ "\n\nKeys: " 
			+ Note.ammo[_song.options.mania] 
			+ "\n\nNotes: "
			+ Song.getNoteCount(_song);
	}
	...
	function updateSelectArrow() {
		...
		if (FlxG.mouse.x > gridBG.x
			...
			if (FlxG.keys.pressed.SHIFT)
				selectionArrow.y = FlxG.mouse.y;
			else
				selectionArrow.y = Math.floor(FlxG.mouse.y / GRID_SIZE) * GRID_SIZE;
			...
			if (selectionArrow.animation.curAnim == null)
				selectionArrow.playAnim('static${selectionArrow.noteData}', false);
			else if (!selectionArrow.animation.curAnim.name.endsWith(Std.string(selectionArrow.noteData)))
				selectionArrow.playAnim('static${selectionArrow.noteData}', false);
		} else {
			selectionArrow.visible = false;
		}
	...
	function updateQuant() {
		curQuantOverlay++;
		if (curQuantOverlay >= quantOverlayData.length)
			curQuantOverlay = 0;
		// updateBpmText();
		reloadGridLayer();
	}
	...
	function reloadGridLayer() { // sex
		...
		for (y in 0...gridQuantOverlay.pixels.height) {
			...
			if ((y / zoomList[curZoom]) % curQuantData[0] < curQuantData[1])
				continue;
	...
	function loadHealthIconFromCharacter(char:String) {
		...
		#end
		{
			path = Paths.getPreloadPath('data/characters/' + Character.DEFAULT_CHARACTER +
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note {
		...
		if (daSus != null) { // Common note
			...
			if (ClientPrefs.settings.get("noteColor") == 'Rainbow')
				note.colorSwap.hue = ((daStrumTime / 5000 * 360) / 360) % 1;
			if (ClientPrefs.settings.get("noteColor") == 'Quant')
				note.checkNoteQuant(note, note.strumTime);
		} else { // Event note
	...
	public function doANoteThing(cs, d, style) {
		...
		if (strumLineNotes.members[d].overlaps(curRenderedNotes)) {
			curRenderedNotes.forEachAlive(function(note:Note) {
				if (note.overlapsPoint(FlxPoint.weak(strumLineNotes.members[d].x + 1, strumLine.y + 1))
		...
		if (gridUpdate) {
			switch (noteData) {
				...
				default:
					...
					note.mustPress = _song.notes[curSection].mustHitSection;
					if (curSelectedNote[1] > Note.ammo[_song.options.mania] - 1)
						note.mustPress = !note.mustPress;
			}
		}
	...
	private function saveLevel() {
		...
		if ((data != null) && (data.length > 0)) {
			#if mobile
			SUtil.saveContent(Paths.formatToSongPath(UI_songTitle.text) + '-' + CoolUtil.difficulties[PlayState.storyDifficulty].toLowerCase() + '.json',
				data.trim());
			#else
	...
	public function playAnim(anim:String, ?force:Bool = false) {
		...
		if (animation.curAnim == null || animation.curAnim.name == 'static$noteData') {
			...
			colorSwap.hue = ClientPrefs.arrowHSV[
				Std.int(Note.keysShit.get(PlayState.mania).get('pixelAnimIndex')[noteData] % Note.ammo[PlayState.mania])
			][0] / 360;
