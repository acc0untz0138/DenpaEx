...
class HscriptClass extends ExtendClass {
	...
	public function new(name:String) { // Used in InitState!!
		...
		for (name => variable in classContainer.vars) {
			if (variable.getter.access == 'get' && variable.getter.func == null)
				throw 'Hscript-Class "${this.name}" variable "$name" requires "get_$name" function!';
			if (variable.setter.access == 'set' && variable.setter.func == null)
				throw 'Hscript-Class "${this.name}" variable "$name" requires "set_$name" function!';
		}
	...
	public function get_(fieldName:String, requiredAccess:Array<FieldAccess>, ?funcArgs:Array<Dynamic> = null):Dynamic {
		...
		if (funcArgs != null) {
			...
			if (func == null)
				throw 'Invalid function parameters! "$name" has no function named "$fieldName"!';

			for (reqAccess in requiredAccess) {
				if (!func.access.contains(reqAccess))
					throw 'Invalid access to "$fieldName", missing access "$reqAccess"!';
			}
			return Reflect.callMethod(func, func.expression, funcArgs);
		} else if (classContainer.funcs.get(fieldName) != null)
			throw 'Invalid access! "$fieldName" requires function parameters!';

		// Check if its available (must be var at this point!)
		final variable = classContainer.vars.get(fieldName);
		if (variable == null)
			throw 'Invalid access! "$name" has no field "$fieldName"!';
		for (reqAccess in requiredAccess) {
			if (!variable.access.contains(reqAccess))
				throw 'Invalid access to "$fieldName", missing access "$reqAccess"!';
		}

		switch (variable.getter.access) {
			case GET:
				return variable.getter.content();
			case NULL:
				if (requiredAccess.length != 0)
					throw 'Cannot get "$fieldName" outside of class "$name" due to getter access "NULL"!';
			case NEVER:
				throw 'Cannot get "$fieldName" with getter access "NEVER"!';
	...
	public function set_(fieldName:String, requiredAccess:Array<FieldAccess>, value:Dynamic) {
		...
		switch (variable.getter.access) {
			case SET:
				variable.val = variable.setter.content(value);
			case NULL:
				if (requiredAccess.length != 0)
					throw 'Cannot set "$fieldName" outside of class "$name" due to setter access "NULL"!';
			case NEVER:
				throw 'Cannot set "$fieldName" with setter access "NEVER"!';
			default: // regular behaviour
		}
		variable.val = value;
	}
