...
class FlxAnimationController implements IFlxDestroyable {
	 ...
	 */
	public function add(Name:String, Frames:Array<Int>, FrameRate:Float = 30, Looped:Bool = true, FlipX:Bool = false, FlipY:Bool = false,
			?LoopPoint:Int = 0):Void {
		...
		if (framesToAdd.length > 0) {
			var anim = new FlxAnimation(this, Name, framesToAdd, FrameRate, Looped, FlipX, FlipY, LoopPoint);
	 ...
	 * @param   LoopPoint    The frame where the looped animation restarts from.
	 */
	public function addByNames(Name:String, FrameNames:Array<String>, FrameRate:Int = 30, Looped:Bool = true, FlipX:Bool = false, FlipY:Bool = false,
			?LoopPoint:Int = 0):Void {
		if (_sprite.frames != null) {
			...
			if (indices.length > 0) {
				var anim = new FlxAnimation(this, Name, indices, FrameRate, Looped, FlipX, FlipY, LoopPoint);
				_animations.set(Name, anim);
			}
		}
	}
	...
	public function addByStringIndices(Name:String, Prefix:String, Indices:Array<String>, Postfix:String, FrameRate:Int = 30, Looped:Bool = true,
		...
		if (_sprite.frames != null) {
			...
			if (frameIndices.length > 0) {
				var anim:FlxAnimation = new FlxAnimation(this, Name, frameIndices, FrameRate, Looped, FlipX, FlipY, LoopPoint);
				_animations.set(Name, anim);
			}
		}
	}
