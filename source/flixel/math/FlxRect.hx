...
class FlxRect implements IFlxPooled {
	 ...
	 * @param rotationOffset Optional rotation offset to be used when calculating the rotated bounds.
	 * @return A globally aligned `FlxRect` that fully contains the input rectangle.
	 * @since 4.11.0
	 */
	public function getRotatedBounds(degrees:Float, ?origin:FlxPoint, ?newRect:FlxRect, ?rotationOffset:FlxPoint):FlxRect {
		if (origin == null)
		...
		if (rotationOffset == null)
			rotationOffset = FlxPoint.weak(0, 0);
		...
		var left = -origin.x - rotationOffset.x;
		var top = -origin.y - rotationOffset.y;
		var right = -origin.x + width - rotationOffset.x;
		var bottom = -origin.y + height - rotationOffset.y;
		if (degrees < 90) {
