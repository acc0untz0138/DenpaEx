...
class FlxCamera extends FlxBasic {
	...
	/**
	 * Converts the camera's `canvas` field into a BitmapData object and returns it.
	 * @param offsets An FlxPoint representing the position offsets for the Matrix (not the actual bitmap getting itself)
	 */
	public function getCanvasBitmap(?offsets:FlxPoint = null):BitmapData {
		if (offsets == null)
			offsets = FlxPoint.get();
		final tempSprite = new FlxSprite();
		tempSprite.makeGraphic(FlxG.width, FlxG.height, FlxColor.TRANSPARENT);
		tempSprite.pixels.draw(camera.canvas, new Matrix(1, 0, 0, 1, -offsets.x, -offsets.y)); // add canvas pixels to a sprite

		return tempSprite.pixels;
	}

	/**
	 * Gets the color information of the pixel positioned at pos.x and pos.y.
	 * @param pos The position at which you want to get the pixel as an FlxPoint.
	 * @param offsets The general offset you wanna set for the matrix (refer to getCanvasBitmap for further help!)
	 * @return The color of the pixel located at the FlxPoint.
	 */
	public function getPixel(pos:FlxPoint, ?offsets:FlxPoint = null):Int {
		return getCanvasBitmap(offsets).getPixel32(Std.int(pos.x), Std.int(pos.y));
	}
	...
	inline function reCalcScrollRect() {
		if (_scrollRect == null)
			return _scrollRect.scrollRect = null;
		...
		return _scrollRect.scrollRect = rect;
	}
	...
	function updateScrollRect():Void {
		...
		final scaledWidth = width * initialZoom * FlxG.scaleMode.scale.x;
		final scaledHeight = height * initialZoom * FlxG.scaleMode.scale.y;

		_scrollRect.x = _scrollRect.y = 0;

		var rect = FlxRect.get();
		rect.copyFromFlash(_scrollRect.scrollRect);
		rect.getRotatedBounds(angle, FlxPoint.get(FlxMath.lerp(rect.left, rect.right, 0.5), FlxMath.lerp(rect.top, rect.bottom, 0.5)), rect);
		_scrollRect.x += rect.x - _scrollRect.scrollRect.x;
		_scrollRect.y += rect.y - _scrollRect.scrollRect.y;
		_scrollRect.scrollRect = rect.copyToFlash();
		rect.put();

		_scrollRect.x -= scaledWidth * 0.5;
		_scrollRect.y -= scaledHeight * 0.5;
	}
	...
	// TODO: IMPLEMENT WHEN WE MAKE RUNTIMES WORK IN A PATCH LMAO!!!

	/**
	 * Convenience function to quickly set specific runtime shaders to a camera.
	 * @param shaders The Shader you want to set for this camera.
	 */
	...
	function checkResize():Void {
		// im sobbing why didnt you just do this before
		if (!FlxG.renderBlit)
			return;
