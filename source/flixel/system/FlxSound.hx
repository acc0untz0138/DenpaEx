...
class FlxSound extends FlxBasic {
	...
	function reset():Void {
		...
		@:privateAccess _focusPreventation = #if FLX_SOUND_SYSTEM FlxG.game != null ? !FlxG.game._lostFocus : #end
		true;
	 ...
	 * @return	This FlxSound instance (nice for chaining stuff together, if you're into that).
	 */
	public function loadStream(SoundURL:String, Looped:Bool = false, AutoDestroy:Bool = false, ?OnComplete:Void->Void, ?OnLoad:Void->Void,
			Pitch:Float = 1):FlxSound {
		cleanup(true);
	...
	function set_pitch(v:Float):Float {
		...
		if (_channel != null) {
			if (_channel.pitch == adjusted)
				return v;
			_channel.pitch = adjusted;
		}
		if (_realPitch == adjusted)
			return v;
