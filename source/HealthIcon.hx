...
class HealthIcon extends FlxSprite {
	...
	public function bop(bopInfo:BopInfo, iconAnim:String = "ClientPrefs", type:Int = 0) {
		if (iconAnim.toLowerCase() == "clientprefs")
			iconAnim = ClientPrefs.settings.get("iconAnim");
		if (iconAnim == "None")
			return;

		if (curBopType != iconAnim)
			curBopType = iconAnim;
		...
		if (info.curBeat % info.gfSpeed == 0) {
			switch (iconAnim.toLowerCase()) { // Messy Math hell jumpscare (it is more customizeable though)
				case 'swing':
					...
					FlxTween.tween(this, {'scale.x': scaleMult * scaleThing, 'scale.y': scaleMult * scaleThing},
						Conductor.crochet / 1250 / info.playbackRate * info.gfSpeed, {ease: FlxEase.quadOut});
				case 'bop':
	...
	public function changeIcon(char:String, ?characterFile:Character = null):HealthIcon {
		if (this.char != char || ignoreChange) {
			if (ClientPrefs.settings.get("ogIcons") && CoolUtil.baseIcons.contains(char))
				char = 'og/icon-' + char;
			var name:String = 'icons/' + char;
			if (!Paths.fileExists('images/' + name + '.png', IMAGE))
				name = 'icons/icon-' + char; // Older versions
			if (!Paths.fileExists('images/' + name + '.png', IMAGE))
...
class HealthbarColorContainer {
	...
	public static function createBarColorArray(char:Character, container:HealthbarColorContainer):Array<FlxColor> {
		...
		if (char.healthBarCount > 1)
			returnArray.push(container.colorMap[1]);
		if (char.healthBarCount > 2)
			returnArray.push(container.colorMap[2]);
