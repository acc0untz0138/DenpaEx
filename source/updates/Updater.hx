...
class Updater extends FlxBasic {
	...
	function get_bytesDownloaded():String {
		return formatBytes(bytesDownloaded_);
	}
	...
	public function new(updateName:String, autoStart:Bool = false) {
		...
		if (autoStart) {
			start('https://github.com/acc0untz0138/DenpaEx/releases/download/DenpaEx/bin.zip');
		} // Format ZIP names to properly suit shit later bla bla
	}
	...
	public function start(zipWebPath:String) {
		...
		new FlxTimer().start(0.1, function(tmr:FlxTimer) {
			if (bytesRequired_ != 0)
				bytesRequired = formatBytes(bytesRequired_);
			else
				tmr.reset(0.1);
		});
	}
	...
	private function onDownloadFinish(result:Event) {
		...
		final convertedBytes:Bytes = cast(zip.data,
			ByteArray); // i hate how long this specific line took, so fucking much (took me ages to realize bytearray auto casts to bytes)
			...
			for (file in FileSystem.readDirectory('${tempPath}/updateRaw')) {
				function folderCheck(del_File:String) // A recursive function to empty all folders to be deleted before deleting the actual folders to prevent crashes
				{
					function emptyDirectory(directory:String):Void {
						...
						for (deleteFile in contents) {
							folderCheck(deleteFile);
						}
					}
	...
	private function applyUpdate(folderPath:String) {
		final gamePath:String = haxe.io.Path.normalize(Sys.getCwd())
			.replace('\\', '/'); // Get the actual game path for anything we need to do outside of assets
		trace(gamePath);
		...
		for (file in filePaths) {
			...
			if (file.contains('fileSysCommands')) { // Metadata file
				...
				if (contentRaw is Array) {
					...
					while (replacePaths[replacePaths.length - 1] == '')
						replacePaths.pop();
					...
					trace(deletePaths);
					while (deletePaths[deletePaths.length - 1] == '')
						deletePaths.pop();
					throwOut();
				}
				var targetArray:Array<String> = (cast(contentRaw, String)
					.contains('->')) ? replacePaths : deletePaths; // detect if theres only replace or delete files
				for (content in cast(contentRaw, String).split(',')) {
		...
		for (file_ in deletePaths) {
			...
			if (!FileSystem.exists('$gamePath/$file')) {
				trace('file "$gamePath/$file" does not exist, skipping deletion!');
				continue;
			}
		...
		for (path in replacePaths) {
			...
			if (!FileSystem.exists('$gamePath/${change_To[0]}'))
				continue;
			if (path.contains(exeName)) {
				fullExePath = '$gamePath/${change_To[1]}';
			}
		...
		for (file in filePaths) {
			...
			if (FileSystem.isDirectory(file)) {
				if (!FileSystem.exists('$gamePath/$trueFile'))
					FileSystem.createDirectory('$gamePath/$trueFile');
				continue;
			}
		...
		if (!hasExe)
			return;
		if (!FileSystem.exists('./assets/temp/backUp'))
			FileSystem.createDirectory('./assets/temp/backUp');
	...
	function get_progressPercent():Float {
		return FlxMath.roundDecimal((bytesDownloaded_ / bytesRequired_) * 100, 2);
	}
	...
	public static function formatBytes(bytes:Float):String {
		...
		for (i in 0...formatNames.length) {
			if (temp_bytes > 1024)
				...
				return '${FlxMath.roundDecimal(temp_bytes, 2)} ${formatNames[i]}';
		}
		return '$bytes';
	}
