...
class CharacterSelectSubstate extends MusicBeatSubstate {
	...
	public function new() {
		...
		if (charactersArray == null || charactersArray.length < 0)
			charactersArray.push("bf");
		for (i in 0...charactersArray.length) {
			if (charactersArray[i].toLowerCase().contains("dead"))
				charactersArray.remove(charactersArray[i]);
		}
		...
		selectedText = new FlxText(0, 65, 0, CoolUtil.toTitleCase(charactersArray[selected].toLowerCase()).replace("Player", " ").trim(),
			32).setFormat("VCR OSD Mono", 32, 0xffffffff, CENTER, OUTLINE, 0xff000000);
	...
	function changeSelection(value:Int = 0) {
		...
		if (selected == 0 && value == -1)
			value = charactersArray.length - 1;
		if (selected == charactersArray.length - 1 && value == 1)
			value = -(charactersArray.length - 1);
		selected += value;
