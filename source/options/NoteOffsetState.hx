...
class NoteOffsetState extends MusicBeatState {
	...
	override public function update(elapsed:Float) {
		...
		rightIcon.x = healthBar.x
			+ (healthBar.width * (FlxMath.remapToRange(50, 0, 100, 100, 0) * 0.01))
			+ (150 * rightIcon.scale.x - 150) / 2
			- iconOffset;
		leftIcon.x = healthBar.x
			+ (healthBar.width * (FlxMath.remapToRange(50, 0, 100, 100, 0) * 0.01))
			- (150 * leftIcon.scale.x) / 2
			- iconOffset * 2;
		...
		for (i in 0...dumbTexts.length) {
			if (controls.mobileC) {
				switch (i) {
					...
					case 4:
						dumbTexts.members[i].text = 'Healthbar Offset:';
					case 5:
						dumbTexts.members[i].text = '[' + ClientPrefs.comboOffset[4] + ', ' + ClientPrefs.comboOffset[5] + ']';
					case 6:
						dumbTexts.members[i].text = 'Controls:';
					case 7:
						dumbTexts.members[i].text = 'LEFT RIGHT UP DOWN:';
					case 8:
						dumbTexts.members[i].text = 'Move Healthbar Offset';
				}
			} else {
				switch (i) {
					case 0:
					...
					case 7:
						dumbTexts.members[i].text = 'LEFT RIGHT UP DOWN:';
					case 8:
						dumbTexts.members[i].text = 'Move Rating Offset';
					case 9:
						dumbTexts.members[i].text = 'A D W S:';
					case 10:
						dumbTexts.members[i].text = 'Move Combo Offset';
					case 11:
						dumbTexts.members[i].text = 'J L I K:';
					case 12:
						dumbTexts.members[i].text = 'Move Healthbar Offset';
				}
			}
