...
class Character extends FlxSprite {
	...
	override function update(elapsed:Float) {
		...
		if (debugMode || (animation == null || animation.curAnim == null))
			return;
		// make this not cancer

		if (heyTimer > 0)
			heyTimer -= elapsed * (PlayState.instance == null ? 1 : PlayState.instance.playbackRate);
			...
			default:
				...
				#end
				{
					path = Paths.getPreloadPath('data/characters/' + DEFAULT_CHARACTER +
						'.json'); // If a character couldn't be found, change him to default just to prevent a crash
				}
				...
				if (!ClientPrefs.settings.get("globalAntialiasing"))
					antialiasing = false;
