...
class CoolUtil {
	...
	public static var baseIcons:Array<String> = [
		'bf',
		'bf-old',
		'bf-pixel',
		'dad',
	...
	inline public static function getMemUsage():Array<Any> {
		...
		return [
			Math.abs(#if cpp Gc.memInfo64(0) #elseif sys cast(cast(System.totalMemory, UInt64), Float) #else 0 #end),
			'B'
		];
	}
	...
	inline public static function toTitleCase(input:String, ?getSpecified:Int):String {
		...
		if (getSpecified == null) {
			...
			if (getSpecified < 0)
				getSpecified = words.length - getSpecified + 1;
			var i = getSpecified;
