...
class CrossFade extends FlxSprite {
	...
	public function resetShit(character:Character, ?isDad:Bool = true) {
		...
		if (colorMap.exists(character.curCharacter)) {
			...
			else if ((character.curCharacter == 'tankman' || character.curCharacter == 'tankman-player')
				&& PlayState.instance.tankmanRainbow)
				index = FlxG.random.int(1, 7);

			color = colors[index];
		} else
			color = FlxColor.subtract(FlxColor.fromRGB(character.healthColorArray[0].red, character.healthColorArray[0].green,
				character.healthColorArray[0].blue), 0x00333333);
		...
		switch (curCrossFadeMode) {
			...
			case 'Eccentric':
				...
				acceleration.x = (velocity.x > 0 ? FlxG.random.int(25,
					75) : FlxG.random.int(-25, -75)) * PlayState.instance.playbackRate * PlayState.instance.playbackRate;
			default:
				velocity.x = (isDad ? (oppositeDir ? -velo : velo) : (oppositeDir ? velo : -velo)) * PlayState.instance.playbackRate;
				acceleration.x = (velocity.x > 0 ? FlxG.random.int(4,
					12) : FlxG.random.int(-4, -12)) * PlayState.instance.playbackRate * PlayState.instance.playbackRate;
		}
