...
class ManualSubState extends MusicBeatSubstate {
	...
	final usableIcons:Array<String> = [
		'bf',
		'dad',
		'gf',
		'mom',
	...
	override function update(elapsed) {
		...
		if (controls.UI_DOWN)
			moveManual(manual.y - (elapsed * mult), scrollSound);
		if (controls.UI_UP)
			moveManual(manual.y + (elapsed * mult), scrollSound);
		if (FlxG.keys.justPressed.HOME)
			moveManual(60, endSound);
		if (FlxG.keys.justPressed.END)
			moveManual((-manual.height + FlxG.height) - 60, endSound);
		if (FlxG.keys.justPressed.PAGEUP)
			moveManual(manual.y + manual.height / 22, pageSound);
	...
	function moveManual(val:Float, snd:FlxSound) {
		...
		moveTwn = FlxTween.tween(manual, {y: val}, val / (val * 1.83), {ease: FlxEase.expoOut, onComplete: (_) -> {
			moveTwn = null;
		}});
