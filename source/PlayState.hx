...
class PlayState extends MusicBeatState {
	...
	public static function set_mania(newMania:Int) {
		...
		function maniaCheck(note:Dynamic) {
			if (note.mania != mania)
				note.mania = mania;
		...
		if (PlayState.instance.sustains != null)
			for (sus in PlayState.instance.sustains)
				maniaCheck(sus);
	...
	function set_cpuControlled(newV:Bool):Bool {
		cpuControlled = newV;
		if (hud != null)
			hud.botplayTxt.visible = cpuControlled;
	...
	public static var lastCritBG:FlxSprite;
	...
	var npfMap:IntMap<Int> = new IntMap(); // NOTES PER FRAME
	var maxNps:Int = 0;
	...
	// Gestionnaire de l'entre-deux
	// public var stateTweens:FlxTweenManager = new FlxTweenManager();
	// Gestionnaire de la minuterie
	public var stateTimers:FlxTimerManager = new FlxTimerManager();

	var missSoundGrp:FlxTypedGroup<FlxSound> = new FlxTypedGroup<FlxSound>();
	...
	var compactUpdateFrame:Int = 0;
	var popUpsFrame:Int = 0;
	var missRecalcsPerFrame:Int = 0;
	var charAnimsFrame:Int = 0;
	var oppAnimsFrame:Int = 0;
	...
	override public function create() {
		...
		keysArray = ClientPrefs.getManiaKeys(mania);
		...
		grpNoteSplashes = new FlxTypedGroup<NoteSplash>((mania +
			1) * 3); // We add a limit so it doesnt cause absurd lag. Since its jsut recyling, we shouldnt have any issues.
		...
		if (stageData.sprites != null && stageData.sprites.length > 0)
			useJsonStage = true;
			...
			switch (curStage) {
				...
				case 'mall': // Week 5 - Cocoa, Eggnog
					...
					if (!ClientPrefs.settings.get("lowQuality")) {
						upperBoppers = [
							...
							new BGSprite('$path/christmas/upperBopRight', 792, -65, 0.3, 0.33, [
								(SONG.header.song.toLowerCase() == 'eggnog' ? 'Upper Crowd Bob right no lemon' : 'Upper Crowd Bob right0')
							])
						];
				...
				case 'school': // Week 6 - Senpai, Roses
					...
					if (!ClientPrefs.settings.get("lowQuality")) {
						...
						for (i in 0...howMany) {
							var schoolClouds = new BGSprite('$path/weeb/weebClouds', FlxG.random.int(isRoses ? -120 : -60, 60),
								FlxG.random.int(isRoses ? -120 : -24, 6), 0.15 + 0.05 * i, 0.2 + 0.01 * i);
							schoolClouds.ID = i;
						...
						if (isRoses) {
							...
							for (i in 0...howMany) {
								var rosesLightning = new BGSprite('$path/weeb/weebLightning', schoolCloudsGrp.members[i].x, schoolCloudsGrp.members[i].y,
									0.15 + 0.05 * i, 0.2 + 0.01 * i);
								rosesLightning.ID = i;
				...
				case 'tank': // Week 7 - Ugh, Guns, Stress
					...
					if (!ClientPrefs.settings.get("lowQuality"))
						foregroundSprites.add(new BGSprite('$path/tank1', -300, 750, 2, 0.2, ['fg']));
					foregroundSprites.add(new BGSprite('$path/tank2', 450, 940, 1.5, 1.5, ['foreground']));
					if (!ClientPrefs.settings.get("lowQuality"))
						foregroundSprites.add(new BGSprite('$path/tank4', 1250, 900, 1.5, 1.5, ['fg']));
					foregroundSprites.add(new BGSprite('$path/tank5', 1620, 700, 1.5, 1.5, ['fg']));
					if (!ClientPrefs.settings.get("lowQuality"))
						foregroundSprites.add(new BGSprite('$path/tank3', 1300, 1200, 3.5, 2.5, ['fg']));
			}
		} else {
		...
		grpGFCrossFade = new FlxTypedGroup<CrossFade>(3);
		...
		strumLine = FlxPoint.get(ClientPrefs.settings.get("middleScroll") ? STRUM_X_MIDDLESCROLL : STRUM_X,
			ClientPrefs.settings.get("downScroll") ? FlxG.height - 150 : 50);
		...
		if (iconP4 != null && iconP1 != null && iconP2 != null)
			iconP4.visible = iconP1.visible = iconP2.visible = ClientPrefs.settings.get("healthBarVisibility");
		if (iconP4.visible && !SONG.assets.enablePlayer4)
			iconP4.active = iconP4.visible = false;
		...
		if (flashLight) {
			var black:FlxSprite = new FlxSprite(0,
				ClientPrefs.settings.get("downScroll") ? flashLightSprite.y : 0).makeGraphic(Math.floor(flashLightSprite.x),
					Math.floor(flashLightSprite.height), FlxColor.BLACK);
			black.scrollFactor.set();
			black.cameras = [camHUD];
			add(black);
			var black2:FlxSprite = new FlxSprite(0,
				ClientPrefs.settings.get("downScroll") ? 0 : black.height).makeGraphic(FlxG.width, Math.floor(FlxG.height - black.height), FlxColor.BLACK);
		...
		final modifierDatas:Array<ModifierSpriteData> = [
			{
				name: 'botplay',
				condition: cpuControlled,
				xPos: 0,
				yPos: 0
			},
			{
				name: 'healthgain',
				condition: healthGain != 1,
				xPos: 0,
				yPos: 1
			},
			{
				name: 'healthloss',
				condition: healthLoss != 1,
				xPos: 0,
				yPos: 2
			},
			{
				name: 'instakill',
				condition: instakillOnMiss,
				xPos: 0,
				yPos: 3
			},
			{
			...
			{
				name: (songSpeedType == 'constant' ? 'scrolltypeconstant' : 'scrolltypemultiplicative'),
				condition: (songSpeedType == 'constant' ? true : ClientPrefs.getGameplaySetting('scrollspeed', 1) != 1),
				xPos: 1,
				yPos: 3
			},
			{
		...
		var foldersToCheck:Array<String> = [
			Paths.getPreloadPath('scripts/songs/' + Paths.formatToSongPath(SONG.header.song) + '/')
		];
		...
		if (canIUseTheCutsceneMother()) {
			...
			switch (daSong) {
				case "winter-horrorland":
					...
					new FlxTimer(stateTimers).start(1.2, function(_) {
					...
					new FlxTimer(stateTimers).start(2.2, function(tmr:FlxTimer) {
						camHUD.visible = true;
						camHUD.alpha = 0;
		...
		add(missSoundGrp);
		for (i in 0...3) {
			var snd = new FlxSound().loadEmbedded(Paths.sound('missnote${i + 1}'));
			FlxG.sound.list.add(snd);
			missSoundGrp.add(snd);
		}
		...
		discordUpdateTimer = new FlxTimer().start(updaterThing, function(tmr:FlxTimer) {
			...
			if (!inCutscene && !cutsceneHandlerCutscene) {
				...
				DiscordClient.changePresence(paused ? 'Paused - ' + detailsText : detailsText + ' - Playing as ' + player,
					SONG.header.song
					+ " ("
					+ storyDifficultyText
					+ ")"
					+ " -"
					+ ((ratingText.toLowerCase().trim() == 'unrated') ? " " + ratingText : " Rating " + ratingText),
					iconP2.char, false);
	...
	public function createRuntimeShader(name:String):FlxRuntimeShader {
		if (!ClientPrefs.settings.get("shaders"))
		...
		if (generatedMusic) {
			try {
				...
				if (secondaryVocals != null)
					secondaryVocals.pitch = value;
				FlxG.sound.music.pitch = value;
			} catch (e) {
				if (vocals != null)
					vocals.pitch = 1;
				if (secondaryVocals != null)
					secondaryVocals.pitch = 1;
	...
	public function reloadHealthBarColors(p4:Bool, ?useGf:Bool = null, ?usePoison:Bool = null, ?otherGfVariable:Bool = null) {
		...
		if (ClientPrefs.settings.get("ogHp"))
			return;
		...
		if (cpuControlled && !ClientPrefs.settings.get('disableBotIcon'))
			who2_CC.setFadingColor(FlxColor.fromRGB(214, 214, 214));
		if (usePoison != null)
			(!opponentChart ? who2_CC : who_CC).setFadingColor(FlxColor.fromRGB(171, 24, 233));
	...
	inline private function generateSong():Void {
		...
		if (SONG.header.vocalsVolume == null)
			SONG.header.vocalsVolume = 1;
		if (SONG.header.secVocalsVolume == null)
			SONG.header.secVocalsVolume = 1;
		if (SONG.header.instVolume == null)
			SONG.header.instVolume = 1;

		FlxG.sound.music.volume = !ffmpegMode ? SONG.header.instVolume : 0;
		...
		if (#if sys FileSystem.exists(Paths.modsJson('charts/' + songName + '/events'))
			|| FileSystem.exists(file) #else OpenFlAssets.exists(file) #end) {
	...
	public function startCountdown():Void {
		...
		switch (SONG.header.song.toLowerCase()) {
			case 'senpai' | 'roses' | 'thorns':
				if (inCutscene) {
					new FlxTimer(stateTimers).start(0.15 / (Conductor.bpm / 100) / playbackRate, function(tmr:FlxTimer) {
						camHUD.alpha += 0.3;
						if (camHUD.alpha < 1)
							tmr.reset(0.15);
					});
				...
				if (SONG.header.song.toLowerCase() == 'senpai')
					setBeatZooms(1, 0); // make sure it starts off fast
		}
		inCutscene = false;
		recalculateIconAnimations(true);
		...
		if (ret != FunkinLua.Function_Stop) {
			...
			startTimer = new FlxTimer(stateTimers).start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer) {
				if (gf != null
				...
				for (char in [boyfriend, dad, player4]) {
					if (char != null
						&& tmr.loopsLeft % char.danceEveryNumBeats == 0
				...
				function countdownSprite(num:Int) {
					...
					add(sprite);
					FlxTween.tween(sprite, {'scale.x': sprite.scale.x * 1.0228991278, 'scale.y': sprite.scale.y * 1.0428991278, alpha: 0},
						Conductor.crochet / 1000 / playbackRate, {
	...
	public function setSongTime(time:Float) {
		...
		if (charterStart) {
			...
			vocals.volume = 0;
			secondaryVocals.volume = 0;
			new FlxTimer(stateTimers).start(0.2, _ -> {
				...
				FlxTween.tween(sprite, {'scale.x': sprite.scale.x * 1.0228991278, 'scale.y': sprite.scale.y * 1.0428991278, alpha: 0}, 0.8 / playbackRate, {
					...
					onComplete: _ -> {
						remove(sprite, true);
						sprite.destroy();
						FlxG.sound.music.volume = !ffmpegMode ? volumes[0] : 0;
						vocals.volume = !ffmpegMode ? volumes[1] : 0;
						secondaryVocals.volume = !ffmpegMode ? volumes[2] : 0;
					}
				});
				FlxG.sound.play(Paths.sound('introGo' + introSoundsSuffix), 0.6);
			});
	...
	function eventPushed(event:EventNote) {
		switch (event.event) {
			...
			case 'Philly Glow':
				blammedLightsBlack = new FlxSprite().makeGraphic(Std.int(FlxG.width / defaultStageZoom * 1.1), Std.int(FlxG.height / defaultStageZoom * 1.1),
					FlxColor.BLACK);
	...
	private function generateStaticArrows(player:Int, ?changingMania:Bool = false):Void {
		for (i in 0...Note.ammo[mania]) {
			...
			if (!skipArrowStartTween && !skipNextArrowTween) {
				if (changingMania == false) {
					...
					FlxTween.tween(babyArrow, {
						y: babyArrow.y + (babyArrow.downScroll ? -40 : 40),
			...
			if (player == 1) {
				if (!opponentChart || opponentChart && ClientPrefs.settings.get("middleScroll"))
	...
	public function changeMania(newValue:Int, fade:Int = 0) {
		...
		if (!isStoryMode) {
			for (i in 0...playerStrums.members.length) {
				...
				if (fade != 0)
					FlxTween.tween(oldStrum, {alpha: 0}, 1, {onComplete: _ -> remove(oldStrum, true)});
				else
					remove(oldStrum, true);
			}
	...
	override function openSubState(SubState:FlxSubState) {
		if (paused) {
			...
			stateTimers.forEach(tmr -> tmr.active = false);
	...
	override function closeSubState() {
		if (paused) {
			...
			stateTimers.forEach(tmr -> tmr.active = true);
	...
	var scoreFormat = ClientPrefs.settings.get("scoreDisplay");
	var frameHitCnt = 0;
	...
	override public function update(elapsed:Float) {
		...
		frameHitCnt = 0; // it's should be reset every frame drawn
		...
		if (updateTime) {
			...
			if (cast(ClientPrefs.settings.get("timeBarType"), String).contains('Elapsed'))
				songCalc = curTime;
		...
		if (orbit) {
			...
			if (orbit_done && !(Math.sin(orbit_sine) >= 0.95 || Math.sin(orbit_sine) <= -0.95))
				orbit_done = false;
		}
		...
		switch (curStage) {
			case 'tank':
				moveTank(elapsed);
			case 'schoolEvil':
				if (!ClientPrefs.settings.get("lowQuality") && bgGhouls.animation.curAnim.finished)
					bgGhouls.visible = false;
			case 'limo':
				if (!ClientPrefs.settings.get("lowQuality")) {
							...
							for (i in 0...dancers.length) {
								if (dancers[i].x < FlxG.width * 1.5 && limoLight.x > (370 * i) + 130) {
									switch (i) {
										case 0 | 3:
											if (i == 0)
												FlxG.sound.play(Paths.sound('vanilla/week4/dancerdeath'), 0.5);

											var diffStr:String = i == 3 ? ' 2 ' : ' ';
											var particle:BGSprite = new BGSprite('vanilla/week4/gore/noooooo', dancers[i].x + 200, dancers[i].y, 0.4, 0.4,
												['hench leg spin' + diffStr + 'PINK'], false);
											grpLimoParticles.add(particle);
											var particle:BGSprite = new BGSprite('vanilla/week4/gore/noooooo', dancers[i].x + 160, dancers[i].y + 200, 0.4,
												0.4, ['hench arm spin' + diffStr + 'PINK'], false);
											grpLimoParticles.add(particle);
											var particle:BGSprite = new BGSprite('vanilla/week4/gore/noooooo', dancers[i].x, dancers[i].y + 50, 0.4, 0.4,
												['hench head spin' + diffStr + 'PINK'], false);
											grpLimoParticles.add(particle);

											var particle:BGSprite = new BGSprite('vanilla/week4/gore/stupidBlood', dancers[i].x - 110, dancers[i].y + 20, 0.4,
												0.4, ['blood'], false);
											particle.flipX = true;
											particle.angle = -57.5;
											grpLimoParticles.add(particle);
		...
		if (#if android FlxG.android.justReleased.BACK #else virtualPad.buttonP.justPressed #end
			|| controls.PAUSE
			&& startedCountdown
			&& canPause) {
		...
		if (scoreFormat == 'Kade' || scoreFormat == 'DenpaEx') {
			if (scoreFormat == 'Kade') {
				hud.scoreTxt.text = 'NPS/MAX: $notesPerSecond/$maxNps | SCORE: ${FlxStringUtil.formatMoney(songScore, false)} | BREAKS: ${FlxStringUtil.formatMoney(songMisses, false)} | ACCURACY: ${Highscore.floorDecimal(ratingPercent * 100, 2)}% | ${ratingFC != "" ? '($ratingFC) ' : ""} $ratingName';
			} else {
			...
			notesPerSecond = 0;
			for (time => npf in npfMap) {
				if (time + 1000 < Conductor.songPosition) npfMap.remove(time);
				notesPerSecond += npf;
			}
			
			maxNps = FlxMath.maxInt(notesPerSecond, maxNps);
		}
		...
		// NOTE SPAWNING BABY!!
		// swap to recycling soon
		if (unspawnNotes[0] != null) {
			final spawnTime:Float = (1750 / songSpeed) / (FlxMath.bound(camHUD.zoom, null,
				1)); // spawns within [time] ms (btw this BARELY edges close enough to the screen to not be too far ahead and not spawning on screen)
			...
			while (unspawnNotes.length > 0
				&& unspawnNotes[notesAddedCount] != null
				&& unspawnNotes[notesAddedCount].strumTime - Conductor.songPosition < (1700 / songSpeed / unspawnNotes[notesAddedCount].spawnTimeMult)) {
				final dunceNote:Note = (unspawnNotes[notesAddedCount].isSustainNote ? sustains : notes).recycle(Note)
					.setupNoteData(unspawnNotes[notesAddedCount]);

				if (ghostMode)
					ghostModeRoutine(dunceNote);
				dunceNote.scrollFactor.set();

				var strumGroup:FlxTypedGroup<StrumNote> = playerStrums;

				if (!dunceNote.mustPress && dunceNote.strum < 2)
					dunceNote.strum = 1;
				else if (dunceNote.mustPress && dunceNote.strum < 2)
					dunceNote.strum = 0;
				else if (dunceNote.strum == 2 && dunceNote.noteType != 'Third Strum')
					dunceNote.strum = (dunceNote.mustPress ? 0 : 1);
				...
				if (dunceNote.strum < 2 && dunceNote.cameras != [camHUD])
				...
				if (cpuControlled && !dunceNote.isSustainNote) {
					...
					tooLate = (Conductor.songPosition > noteKillOffset + dunceNote.strumTime);
				...
				if (tooLate) {
					...
					if (dunceNote.mustPress
						&& !cpuControlled
						&& !dunceNote.ignoreNote
						&& !endingSong
						&& (dunceNote.tooLate || !dunceNote.wasGoodHit))
						noteMiss(dunceNote);
		...
		if (generatedMusic) {
			...
			// var fakeCrochet:Float = (60 / Conductor.bpm) * 1000; // unused value

			// fuck off man
			if (!inCutscene && !cutsceneHandlerCutscene) {
				for (group in [notes, sustains])
					group.forEachAlive(daNote -> {
						...
						if (cpuControlled && !daNote.isSustainNote) {
							...
							tooLate = (Conductor.songPosition > Math.min(Conductor.stepCrochet, noteKillOffset) + daNote.strumTime);
				...
				if (!ClientPrefs.settings.get("downScroll")) {
					for (group in [notes, sustains])
						inline group.members.sort((a:Note, b:Note) -> inline Std.int(a.y - b.y));
				} else {
					for (group in [notes, sustains])
						inline group.members.sort((b:Note, a:Note) -> inline Std.int(a.y - b.y));
				}
			}
		}

		// Add nps value (i hope the fps doesn't go over 1000 fps)
		if (frameHitCnt > 0) {
			npfMap.set(Math.floor(Conductor.songPosition), frameHitCnt);
		}
		...
		if (hud.botplayTxt != null && hud.botplayTxt.visible) {
			if (ffmpegInfo)
				hud.botplayTxt.text = CoolUtil.floatToStringPrecision(haxe.Timer.stamp() - takenTime, 3);
		}
	...
	public function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((practiceMode || (!opponentChart ? health > 0 : health < 2)) && !skipHealthCheck) || isDead)
			return false;
		...
		openSubState(new GameOverSubstate(boyfriend.getScreenPosition().x - boyfriend.positionOffset.x,
			boyfriend.getScreenPosition().y - boyfriend.positionOffset.y, camFollowPos.x, camFollowPos.y));
		...
		switch (eventName) {
			...
			case 'Philly Glow':
				...
				if (Math.isNaN(lightId))
					lightId = 0;
				if (Math.isNaN(colorInt) || colorInt == 0)
					colorInt = 1;
				...
				switch (lightId) {
					...
					case 1: // turn on
						...
						if (colorInt != 1)
							color = FlxColor.fromInt(colorInt);

						if (!phillyGlowGradient.alive)
							phillyGlowGradient.revive();
						if (!phillyGlowGradient.visible) {
							doFlash();
							if (ClientPrefs.settings.get("camZooms")) {
								FlxG.camera.zoom += 0.5;
			...
			case 'BG Freaks Expression':
				if (bgGirls != null)
					bgGirls.forEach(bgGirl -> bgGirl.swapDanceType());
			...
			case 'Stage Tint':
				...
				new FlxTimer(stateTimers).start(val2 / playbackRate, function(tmr:FlxTimer) {
			...
			case 'Build Up Tint':
				...
				FlxTween.tween(tint, {alpha: 0.95}, val1 / playbackRate,
					{onComplete: _ -> FlxTween.tween(tint, {alpha: 0}, 0.1, {onComplete: _ -> remove(tint, true)})});
			...
			case 'Add Subtitle':
				if (!ClientPrefs.settings.get("subtitles"))
					return;
				...
				switch (split[0].toLowerCase()) {
					...
					case 'bficon' | 'bf' | 'boyfriend' | 'boyfriendicon':
						funnyColor = FlxColor.fromRGB(boyfriend.healthColorArray[0].red, boyfriend.healthColorArray[0].green,
							boyfriend.healthColorArray[0].blue);
						useIco = true;
				...
				var subBG:FlxSprite = new FlxSprite(0,
					ClientPrefs.settings.get("downScroll") ? hud.healthBar.y + 90 : hud.healthBar.y - 90).makeGraphic(Std.int(sub.width + 10),
						Std.int(sub.height + 10), FlxColor.BLACK);
				...
				new FlxTimer(stateTimers).start(stepsToSecs(val3), function(timer:FlxTimer) {
					FlxTween.tween(sub, {alpha: 0}, stepsToSecs(1), {
						ease: FlxEase.quadInOut,
						onComplete: _ -> {
							remove(sub, true);
							sub.destroy();
					...
					FlxTween.tween(subBG, {alpha: 0}, stepsToSecs(1), {
						ease: FlxEase.quadInOut,
						onComplete: _ -> {
							remove(subBG, true);
							subBG.destroy();
	...
	public function finishSong(?ignoreNoteOffset:Bool = false):Void {
		...
		if (!ffmpegMode) {
			if (ClientPrefs.settings.get("noteOffset") <= 0 || ignoreNoteOffset)
				...
				finishTimer = new FlxTimer(stateTimers).start(ClientPrefs.settings.get("noteOffset") / 1000, _ -> finishCallback());
		} else
			finishCallback();
	}
	...
	public function endSong():Void {
		...
		camZooming = false;
		updateTime = false;
		canPause = hitbox.visible = #if !android virtualPad.visible = #end
		false;
		...
		if (canIUseTheCutsceneMother(true) && !seenEnd) {
			...
			switch (SONG.header.song.toLowerCase()) {
				case 'roses':
					...
					camHUD.visible = false;
					if (rainSound != null)
						rainSound.fadeOut(0.7, 0, function(twn:FlxTween) {
							rainSound.stop();
							rainSound = null;
						});
		...
		if (ret != FunkinLua.Function_Stop && !transitioning) {
			if (!chartingMode) {
				...
				if (Math.isNaN(percent))
					percent = 0;
				Highscore.saveScore(SONG.header.song, songScore, storyDifficulty, percent, ratingName);
			}
			playbackRate = 1;
			...
			if (isStoryMode) {
				...
				if (storyPlaylist.length <= 0) {
					...
					#if sys
					if (sys.FileSystem.exists(Paths.modsJson('charts/' + songLowercase + '/' + poop))
						|| sys.FileSystem.exists(Paths.json('charts/' + songLowercase + '/' + poop)))
	...
	public function cachePopUpScore() {
		...
		if (SONG.options.crits)
			Paths.image('${ratingsPath}critBG${suffix}');
		for (i in 0...10)
			Paths.image('${numsPath}num${i}${skinOverride}');
	}

	public function popUpScore(note:Note):String {
		final msTiming:Float = note.strumTime - Conductor.songPosition + ClientPrefs.settings.get("ratingOffset");
		final noteDiff = Math.abs(msTiming);

		if (SONG.header.needsVoices && !ffmpegMode)
			secondaryVocals.volume = vocals.volume = SONG.header.vocalsVolume;
		var score:Int = 0;

		final daRating:String = Conductor.judgeNote(note, noteDiff / playbackRate, cpuControlled);
		switch (daRating) {
			case "wtf":
				if (sickOnly)
					health -= 5;
		...
		if (daRating == 'wtf' || daRating == 'shit') {
			if (gf != null && combo > 5 && gf.animOffsets.hasKey('sad'))
				gf.playAnim('sad');
		...
		if (!practiceMode && !cpuControlled) {
			...
			if (ClientPrefs.settings.get("scoreZoom"))
				hud.scoreTween(daRating);
		}
		recalculateRating();

		spawnRatingDisplay(daRating, msTiming, note);

		return daRating;
	}

	private function spawnRatingDisplay(_rating:String, timing:Float, note:Note) {
		final showRating:Bool = ClientPrefs.settings.get("ratingVisibility") && !ClientPrefs.settings.get("hideHud");

		final crit = FlxG.random.bool(_rating == 'perfect' ? 10 : 1);
		...
		var rating = new FlxSprite().loadGraphic(Paths.image('${ratingsPath}${_rating}${suffix}'));
		...
		FlxTween.tween(rating, {alpha: 0}, 0.2 / playbackRate, {
			...
			onComplete: _ -> {
				remove(rating, true);
				rating = FlxDestroyUtil.destroy(rating);
			}
		});

		if (ClientPrefs.settings.get("msPopup")) {
			msTxt.visible = true; // yanni bro you cant just remove the whole visible = line :sob:
			msTxt.text = (Math.fround(-timing * 100) / 100) + " MS";
			if (msTimer != null)
				msTimer.cancel();
			msTimer = new FlxTimer(stateTimers).start(0.2 + (Conductor.crochet * 0.0005 / playbackRate), _ -> {
				msTxt.text = '';
				msTxt.visible = false;
			});
			switch (_rating) {
		...
		if (crit && SONG.options.crits) {
			var critBG = new FlxSprite().loadGraphic(Paths.image('${ratingsPath}critBG${suffix}'));
			critBG.cameras = (ClientPrefs.settings.get("wrongCamera") ? [camGame] : [camHUD]);
			critBG.screenCenter();
			critBG.x = offset - 150;
			if (ClientPrefs.settings.get("wrongCamera")) {
				critBG.y += boyfriend.y;
				critBG.x += boyfriend.x;
				critBG.y -= isPixelStage ? 400 : 250;
				critBG.x -= isPixelStage ? 800 : 600;
				if (curStage == 'limo')
					critBG.acceleration.x = 750 * playbackRate * playbackRate;
			}
			critBG.y += 80;
			critBG.x += ClientPrefs.comboOffset[2];
			critBG.y -= ClientPrefs.comboOffset[3];

			if (!PlayState.isPixelStage) {
				var floater:Float = 0.6;
				floater += (Math.floor(Math.log(combo) / Math.log(10))) * 0.1;
				critBG.setGraphicSize(Std.int(critBG.width * floater));
			} else {
				critBG.setGraphicSize(Std.int(critBG.width * daPixelZoom));
				critBG.antialiasing = false;
			}
			critBG.updateHitbox();

			critBG.moves = true;
			critBG.acceleration.y = FlxG.random.int(200, 300) * playbackRate * playbackRate;
			critBG.velocity.y -= FlxG.random.int(140, 160) * playbackRate;
			critBG.velocity.x = FlxG.random.float(-5, 5) * playbackRate;

			insert(members.indexOf(strumLineNotes), critBG);
			if (lastCritBG != null) {
				FlxTween.cancelTweensOf(lastCritBG);
				remove(lastCritBG, true);
				lastCritBG.destroy();
			}
			lastCritBG = critBG;

			FlxTween.tween(critBG, {alpha: 0}, 0.2 / playbackRate, {
				onComplete: _ -> {
					remove(critBG, true);
					critBG = FlxDestroyUtil.destroy(critBG);
				},
				startDelay: Conductor.crochet * 0.002 / playbackRate
			});
		}
		...
		var scoreNums:Array<FlxSprite> = [];
		for (daLoop => i in seperatedScore) {
			var numScore = new FlxSprite().loadGraphic(Paths.image('${numsPath}num${i}${skinOverride}'));
			...
			FlxTween.tween(numScore, {alpha: 0}, 0.2 / playbackRate, {
				onComplete: _ -> {
					...
					numScore = FlxDestroyUtil.destroy(numScore);
				},
				startDelay: Conductor.crochet * 0.002 / playbackRate
			});

			scoreNums.push(numScore);
		}

		callOnLuas('onRatingSpawn', [members.indexOf(rating), _rating, timing]);
		callOnHscripts('onRatingSpawn', [rating, scoreNums, seperatedScore, _rating, timing, note]);
	}

	private var sustainLaneCheck:Array<Bool> = [];

	public function onKeyPress(event:KeyboardEvent) {
		var eventKey:FlxKey = event.keyCode;
		var key:Int = getKeyFromEvent(eventKey);
		if (key == -1 || cpuControlled || paused || !FlxG.keys.checkStatus(eventKey, JUST_PRESSED))
			return;

		/*
						hi hi it's me AT (BlueVapor1234)
						i am proud of how this turned out
						credit me if you use this :)
						even though you are required by liscence to not use the code -
						- without credit and permission but still
					*/

		// make sure we arent do weird shit lol
		if (generatedMusic && !boyfriend.stunned && !endingSong) {
			...
			for (note in notes.members) {
				...
				if (note.noteData != key || note.wasGoodHit || note.tooLate || !note.canBeHit)
					continue;
		...
		if (playerStrums.members[key] == null)
			return;
		if (playerStrums.members[key].animation.curAnim.name == 'confirm')
			return;
	...
	// rewrite the following 2 functions later
	public function getKeyFromEvent(key:FlxKey):Int {
		if (key != NONE)
			for (i => collection in keysArray)
				for (keybind in collection)
					if (key == keybind)
						return i;
	...
	public function keysArePressed():Bool {
		for (collection in keysArray)
			for (keybind in collection)
				if (FlxG.keys.checkStatus(keybind, PRESSED))
					return true;
	...
	// Hold notes
	public function sustainInputCheck():Bool {
		if (boyfriend.stunned)
			return false;

		var hitNote:Bool = false;
		for (i in 0...sustainLaneCheck.length) {
			if (!sustainLaneCheck[i])
				continue;

			for (sus in sustains.members) {
				if (sus.noteData != i || !sus.canBeHit || sus.tooLate || sus.wasGoodHit)
					continue;

				// stop you from holding sustains infinitely by just holding the key forever
	...
	public function onHintPress(hint:FlxButton, id:Int) {
		if (hint == null || id == -1 || cpuControlled || paused || !hint.justPressed)
			return;
		...
		if (generatedMusic && !boyfriend.stunned && !endingSong) {
			...
			for (note in notes.members) {
				// check if its in the lane, if it wasnt already hit, that its not late, and that it can be hit
				if (note.noteData != id || note.wasGoodHit || note.tooLate || !note.canBeHit)
					continue;
		...
		// make sure we arent calling a null strum, or a strum that just pressed a valid note
		if (playerStrums.members[id] == null)
			return;
		if (playerStrums.members[id].animation.curAnim.name == 'confirm')
			return;
	...
	public function onHintRelease(hint:FlxButton, id:Int) {
		if (hint == null || id == -1 || cpuControlled || paused)
			return;
	...
	public function noteMiss(daNote:Note) { // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		if (daNote.isSustainNote && !ClientPrefs.settings.get("sustainsAreNotes"))
			return;
		...
		if (ClientPrefs.settings.get("flinching")) {
			final time:Float = (poison ? 3 : 0.5);
			flinching = true;
			recalculateIconAnimations();
			if (flinchTimer != null)
				flinchTimer.cancel();
			flinchTimer = new FlxTimer(stateTimers).start(time, _ -> {
				flinching = false;
				recalculateIconAnimations();
			});
		}
		...
		if (freeze)
			freezeRoutine();
		songMisses++;
		if (ClientPrefs.settings.get("missSoundShit"))
			playMissSound();
		...
		if (char != null) {
			...
			if (char.animOffsets.hasKey(animToPlay)) {
				char.playAnim(animToPlay, true);
			} else {
				char.playAnim('singUP' + missStr + daAlt, true);
			}
		}
	}

	public function noteMissPress(direction:Int = 1) { // You pressed a key when there was no notes to press for this key
		if (tappy)
			return;
		...
		if (ClientPrefs.settings.get("flinching")) {
			...
			if (flinchTimer != null)
				flinchTimer.cancel();
			flinchTimer = new FlxTimer(stateTimers).start(0.5, _ -> {
				flinching = false;
				recalculateIconAnimations();
			});
		}
	...
	function playMissSound() {
		vocals.volume = 0;
		final rand:Int = FlxG.random.int(0, 2);
		for (i in 0...missSoundGrp.members.length) {
			if (i == rand) {
				missSoundGrp.members[i].stop();
				missSoundGrp.members[i].volume = FlxG.random.float(0.1, 0.2);
				missSoundGrp.members[i].play();
				missSoundGrp.members[i].pitch = FlxG.random.float(0.95, 1.05);
			}
		}
	}
	...
	function gsTap(direction:Int = 1, ?miss:Bool = false):Void // GS Tap Miss
	{
		...
		if (ClientPrefs.settings.get("flinching") && miss) {
			...
			flinchTimer = new FlxTimer(stateTimers).start(0.5, function(tmr:FlxTimer) {
				flinching = false;
				recalculateIconAnimations();
			});
		}
	...
	public function opponentNoteHit(note:Note, p4:Bool = false) {
		...
		if (note.noteType == 'Hey!') {
			...
			var altAnim:String = ((SONG.notes[curSection] != null
				&& (SONG.notes[curSection].altAnim || note.noteType == 'Alt Animation')
				&& !SONG.notes[curSection].gfSection) ? '-alt' : '');
			...
			if (oppChar != null) {
				...
				if (ClientPrefs.settings.get("camPans"))
					camPanRoutine(animToPlay, (!opponentChart ? (oppChar == dad ? 'oppt' : 'p4') : 'bf'));
			}

			function makeCrossFade(_char:Character, _grp:FlxTypedGroup<CrossFade>, ?noteTypeThing:Bool = false) {
				if (ClientPrefs.settings.get("crossFadeMode") == 'Off' || note.isSustainNote || noteTypeThing)
					return;
			...
			if (oppChar.scareBf && boyfriend != null && boyfriend.animOffsets.hasKey('scared'))
		...
		if (oppAnimsFrame - 1 < 4)
			oppChar = !note.gfNote ? (!p4 ? (!opponentChart ? dad : boyfriend) : player4) : gf;

		if (ClientPrefs.settings.get('strumVisibility') && strumAnimsPerFrame[0] < 4) {
			strumAnimsPerFrame[0] += 1;
			var time:Float = 0.15;
			if (note.isSustainNote && note.animation.curAnim != null && !note.isSustainEnd)
				time += 0.15;
			if (note.colorSwap != null && ClientPrefs.settings.get("noteColor") != 'Normal')
				hsb = [note.colorSwap.hue, note.colorSwap.saturation, note.colorSwap.brightness];
		...
		callOnLuas('opponentNoteHit', [
			...
			note.noteType,
			note.isSustainNote,
			oppChar.curCharacter
		]);
		callOnHscripts('opponentNoteHit', [
			notes.members.indexOf(note),
			Math.abs(note.noteData),
			note.noteType,
			note.isSustainNote,
	...
	public function goodNoteHit(note:Note, p4:Bool = false) {
		...
		if ((scoreFormat == 'Kade' || scoreFormat == 'DenpaEx') && !note.isSustainNote)
			frameHitCnt++;
		...
		if (!cpuControlled) { // no botplay
			...
			if (!note.isSustainNote && !note.ratingDisabled) {
				...
				if (popUpsFrame < 4) {
					popUpsFrame++;
					final ratingString:String = popUpScore(note);
				}
			}
			...
			if (!note.isSustainNote
				...
				&& ClientPrefs.settings.get("noteSplashes")
				|| note.forceNoteSplash
				&& ClientPrefs.settings.get("noteSplashes"))
				spawnNoteSplashOnNote(note);
		}

		if (!note.isSustainNote || (note.isSustainNote && ClientPrefs.settings.get("sustainsAreNotes")))
		...
		if (!note.noAnimation && charAnimsFrame < 4) {
			charAnimsFrame++;
			final animToPlay:String = 'sing' + Note.keysShit.get(mania).get('anims')[note.noteData];
			...
			if (note.noteType == 'Hey!' && !note.isSustainNote) {
				...
				if (char != gf && gf != null && gf.animOffsets.hasKey('cheer')) {
					gf.playAnim('cheer', true);
					gf.specialAnim = true;
					gf.heyTimer = 0.6;
				}
			...
			function makeCrossFade(?_gf:Bool = false, ?noteTypeThing:Bool = false) {
				...
				switch (_gf) {
					case true:
						var crossfade = grpGFCrossFade.recycle(CrossFade);
		...
		if (cpuControlled && ClientPrefs.settings.get('strumVisibility') && strumAnimsPerFrame[1] < 4) {
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		if (note != null) {
			if (ClientPrefs.settings.get("noteColor") == 'Normal') {
				if (note.noteSplashTexture.length > 0)
	...
	inline public function fastCarDrive() {
		...
		carTimer = new FlxTimer(stateTimers).start(2, function(tmr:FlxTimer) {
			resetFastCar();
			carTimer = null;
		});
	}
	...
	inline public function killHenchmen():Void {
		if (ClientPrefs.settings.get("lowQuality") || curStage != 'limo' || limoKillingState >= 1)
			return;
		limoMetalPole.x = -400;
	...
	public function rosesLightningStrike():Void {
		...
		if (!ClientPrefs.settings.get("lowQuality")) {
			...
			for (rosesLightning in rosesLightningGrp) {
				if (rosesLightning.ID == fuck) {
					...
					FlxTween.tween(rosesLightning, {alpha: 0.001}, 0.75 / playbackRate,
						{startDelay: 0.15 / playbackRate, onComplete: _ -> rosesLightning.visible = false});
				}
			}
		...
		if (ClientPrefs.settings.get("flashing")) {
			...
			FlxTween.tween(halloweenWhite, {alpha: 0.001}, 0.25 / playbackRate,
				{startDelay: 0.15 / playbackRate, onComplete: _ -> halloweenWhite.visible = false});
		}
	}
	...
	override public function destroy() {
		...
		for (grp in [grpCrossFade, grpBFCrossFade, grpGFCrossFade, grpP4CrossFade]) {
			for (crossfade in grp.members) {
				grp.remove(crossfade, true);
				crossfade = FlxDestroyUtil.destroy(crossfade);
			}
		}
		...
		if (ffmpegMode && FlxG.fixedTimestep) {
			...
			case 'guns':
				switch (curStep) {
					case 896:
						...
						for (object in stageGraphicArray)
							if (object != null)
								FlxTween.tween(object, {y: object.y + 820}, 1.35 / playbackRate, {ease: FlxEase.expoInOut});
						if (gf != null)
							FlxTween.tween(gf, {y: gf.y + 840}, 1.35 / playbackRate, {ease: FlxEase.expoInOut});
			...
			case 'thorns':
				switch (curStep) {
					...
					case 48:
						...
						new FlxTimer(stateTimers).start(stepsToSecs(2), function(tmr:FlxTimer) {
			...
			case 'senpai':
				...
				function setLoveGradient(alpha_:Float, time:Float, active:Bool = true) {
					...
					for (loveGradient in senpaiLoveGrp) { // using the poisonSprites because they work!!
						...
						if (active)
							FlxTween.tween(loveGradient, {alpha: alpha_}, time);
					}
				}
				...
				switch (curStep) { // a lot of colouring and zooming because it just fits, sob over it
					...
					case 896:
						...
						new FlxTimer(stateTimers).start(stepsToSecs(4), function(_:FlxTimer) {
							camHUD.alpha -= 0.25;
					...
					case 916:
						...
						new FlxTimer(stateTimers).start(stepsToSecs(1) / 2, function(_:FlxTimer) {
					...
					case 924:
						if (isStoryMode) { // only do this on story mode because it doesnt make sense on freeplay
							...
							new FlxTimer(stateTimers).start(stepsToSecs(2), _ -> blackScreen.alpha += 0.25, 4);
						}
				}
			case 'roses':
				switch (curStep) {
	...
	override public function beatHit() {
		...
		if (beatDrainInterval > 0 && curBeat % beatDrainInterval == beatHitDelay && intendedHealth > beatDrainFloor)
			intendedHealth -= beatDrainDecrease; // for senpai charm mainly
		...
		if (beatZoomingInterval > 0) {
			if (camZooming
				...
				&& ClientPrefs.settings.get("camZooms")
				&& curBeat % beatZoomingInterval == beatHitDelay) {
		...
		final curInfo:HealthIcon.BopInfo = {
			curBeat: curBeat,
			playbackRate: playbackRate,
			gfSpeed: gfSpeed,
			healthBarPercent: hud.healthBar.percent
		};
		var bopString:String = "";
		for (i in 0...iconsArray.length) {
			if (!iconsArray[i].visible)
				continue;
		...
		switch (curStage) {
				...
				switch (SONG.header.song.toLowerCase()) {
					case 'guns':
						if (curBeat % 4 == 0 && tankmanRainbow && gunsThing != null) {
							...
							if (gunsColorIncrementor > 5)
								gunsColorIncrementor = 0;
						}
				}
			case 'school':
				if (!ClientPrefs.settings.get("lowQuality"))
					bgGirls.forEach(bgGirl -> bgGirl.dance());
			case 'mall':
				if (!ClientPrefs.settings.get("lowQuality")) {
					for (bopper in upperBoppers)
						bopper.dance(true);
			...
			case 'limo':
				if (!ClientPrefs.settings.get("lowQuality"))
					grpLimoDancers.forEach(dancer -> dancer.dance());
				if (FlxG.random.bool(10) && fastCarCanDrive)
					fastCarDrive();
				if (FlxG.random.bool(5) && billBoardCanBill)
					billBoardBill();
		}
		...
		if (generatedMusic) { // prevent random null ref (it already happened so this is infact not useless)
			if ((curStage == 'school')
				&& SONG.header.song.toLowerCase() == 'roses'
				&& FlxG.random.bool(10)
	...
	if (doPush) {
		for (hscript in hscripts) {
			if (hscript.scriptName == hscriptFile)
				return;
		}
	...
	switch (ClientPrefs.settings.get("scoreDisplay")) {
		...
		case 'Psych':
			...
			hud.scoreTxt.text = 'Score: ${FlxStringUtil.formatMoney(songScore, false)} | Breaks: ${FlxStringUtil.formatMoney(songMisses, false)} | Rating: $ratingName'
				+ ext;
		case 'Kade':
		...
		case 'FPS+':
			hud.scoreTxt.text = 'Score: ${FlxStringUtil.formatMoney(songScore, false)} | Breaks: ${FlxStringUtil.formatMoney(songMisses, false)} | Accuracy: ${Highscore.floorDecimal(ratingPercent * 100, 2)}%';
		case 'FNF+':
			hud.rightTxt.text = 'HP\n${hud.healthBar.roundedPercent}%\n\nACCURACY\n${Highscore.floorDecimal(ratingPercent * 100, 2)}%\n\nSCORE\n${FlxStringUtil.formatMoney(songScore, false)}';
		case 'Vanilla':
			hud.rightTxt.text = 'Score:${FlxStringUtil.formatMoney(songScore, false)}';
	...
	switch (iconP1.type) {
		case SINGLE:
			iconP1Poison.animation.curAnim.curFrame = iconP1.animation.curAnim.curFrame = 0;
		case WINNING:
			iconP1Poison.animation.curAnim.curFrame = iconP1.animation.curAnim.curFrame = (hud.healthBar.percent > 80 ? 2 : (hud.healthBar.percent < 20 ? 1 : 0));
		default:
			iconP1Poison.animation.curAnim.curFrame = iconP1.animation.curAnim.curFrame = (hud.healthBar.percent < 20 ? 1 : 0);
	}
	switch (iconP2.type) {
		case SINGLE:
			iconP2Poison.animation.curAnim.curFrame = iconP2.animation.curAnim.curFrame = 0;
		case WINNING:
			iconP2Poison.animation.curAnim.curFrame = iconP2.animation.curAnim.curFrame = (hud.healthBar.percent > 80 ? 1 : (hud.healthBar.percent < 20 ? 2 : 0));
		default:
			iconP2Poison.animation.curAnim.curFrame = iconP2.animation.curAnim.curFrame = (hud.healthBar.percent > 80 ? 1 : 0);
	}
	if (flinching) {
		if (poison) {
			if (!ClientPrefs.settings.get("hideHud"))
				(!opponentChart ? iconP1Poison : iconP2Poison).visible = true;
			reloadHealthBarColors(SONG.notes[curSection].player4Section, null, true);
		}
		if ((!opponentChart ? iconP1 : iconP2).type != SINGLE)
			(!opponentChart ? iconP1Poison : iconP2Poison).animation.curAnim.curFrame = (!opponentChart ? iconP1 : iconP2).animation.curAnim.curFrame = 1;
	}
	switch (iconP4.type) {
		case SINGLE:
			iconP4.animation.curAnim.curFrame = 0;
		case WINNING:
			iconP4.animation.curAnim.curFrame = (hud.healthBar.percent > 80 ? 1 : (hud.healthBar.percent < 20 ? 2 : 0));
		default:
			iconP4.animation.curAnim.curFrame = (hud.healthBar.percent > 80 ? 1 : 0);
	}
	}
	...
	iconP1.x = (hud.healthBar.x
		+ (hud.healthBar.width * (FlxMath.remapToRange(offsetter, 0, 100, 100, 0) * 0.01))
	...
	if (target == null || target == player4 && !SONG.assets.enablePlayer4)
		return false; // workaround for p4 not existing occasionally
	...
	if (!killTrailOf(target))
		return; // null check but shorter

	if (target.trailData.enabled && target.trailData.length != null && target.trailData.delay != null && target.trailData.alpha != null
		&& target.trailData.diff != null) {
	...
	final quartizBlends:Array<BlendMode> = [
		ADD,
		ALPHA,
		DARKEN,
		ERASE,
	...
	if (FlxG.random.bool(12.5)) {
		...
		switch (FlxG.random.int(0, 3)) {
			case 0:
				FlxTween.tween(quartiz, {x: quartiz.x + FlxG.random.int(~num + 1, num)}, FlxG.random.float(1, 10),
					{ease: FlxEase.quadInOut, type: FlxTweenType.PINGPONG});
			case 1:
				FlxTween.tween(quartiz, {y: quartiz.y + FlxG.random.int(~num + 1, num)}, FlxG.random.float(1, 10),
					{ease: FlxEase.quadInOut, type: FlxTweenType.PINGPONG});
			case 2:
				FlxTween.tween(quartiz, {angle: quartiz.angle + FlxG.random.int(~num + 1, num)}, FlxG.random.float(1, 10),
					{ease: FlxEase.quadInOut, type: FlxTweenType.PINGPONG});
			case 3:
				FlxTween.tween(quartiz, {x: quartiz.x + FlxG.random.int(~num + 1, num), y: quartiz.y + FlxG.random.int(~num + 1, num)},
					FlxG.random.float(1, 10), {ease: FlxEase.quadInOut, type: FlxTweenType.PINGPONG});
		}
	}
	add(quartiz);
	}
	...
	for (poisonSprite in poisonSpriteGrp) {
		...
		if (poisonSprite.alpha < 1)
			FlxTween.tween(poisonSprite, {alpha: 1}, 0.2);
	}
	if (poisonTimer != null) {
		poisonTimer.cancel();
		poisonTimer = null;
	...
	for (freezeSprite in freezeSpriteGrp) {
		...
		if (freezeSprite.alpha < 1)
			FlxTween.tween(freezeSprite, {alpha: 1}, 0.4);
	}
	...
	boyfriend.stunned = true;
	freezeTimer = new FlxTimer(stateTimers).start(2, _ -> {
		...
		freezeCooldownTimer = new FlxTimer(stateTimers).start(0.2, _ -> {
	...
	if (!daNote.isSustainNote)
		FlxTween.tween(daNote, {alpha: 0}, 0.26 / songSpeed, {startDelay: 0.95 / songSpeed});
	else
		FlxTween.tween(daNote, {multAlpha: 0}, 0.13 / songSpeed, {startDelay: 0.95 / songSpeed});
	}

	public function canIUseTheCutsceneMother(endCutscene:Bool = false):Bool {
	if (seenCutscene && !endCutscene)
		return false;
	...
	function camPanRoutine(anim:String = 'singUP', who:String = 'bf') {
	if (SONG.notes[curSection] == null)
		return;

	var fps:Float = ClientPrefs.settings.get("framerate");
	...
	new FlxTimer(stateTimers).start(0.3, tmr -> {
		...
		if (black.alpha > 0)
			...
			if (Paths.formatToSongPath(SONG.header.song) == 'thorns') {
				...
				new FlxTimer(stateTimers).start(0.3, swagTimer -> {
					...
					if (senpaiEvil.alpha < 1)
						...
						new FlxTimer(stateTimers).start(3.2, _ -> FlxG.camera.fade(FlxColor.WHITE, 1.6, false));
					}
				});
			} else
				add(dialogueBox);
	...
	if (canIUseTheCutsceneMother(true)) {
		...
		var mult = 1;
		new FlxTimer(stateTimers).start(0.05, _ -> {
		...
		new FlxTimer(stateTimers).start(0.45, t -> {
	...
	for (spriteData in stageData.sprites) {
		if (spriteData.hide_lq && ClientPrefs.settings.get("lowQuality"))
			continue;
		...
		if (!spriteData.animated) {
			...
			try {
				...
				for (animationData in stageData.animations[spriteData.animation_index]) {
					leSprite.animation.addByPrefix(animationData.name, animationData.xml_prefix, animationData.framerate, animationData.looped,
						animationData.flip_x, animationData.flip_y);
				}
		...
		if (spriteData.origin != null)
			leSprite.origin.set(spriteData.origin[0], spriteData.origin[1]);
	...
	var args:Array<String> = [
		...
		'-',
		'-c:v',
		codecList[Std.string(ClientPrefs.settings.get("videoEncoder"))],
		'-b',
		Std.string(ClientPrefs.settings.get("renderBitrate") * 1000000),
	...
	if (gpuAccel) {
		if (tmp.contains("QSV"))
			accel = "qsv";
		else if (tmp.contains("NVENC"))
			accel = "cuda";
		else if (tmp.contains("AMF")) {
			accel = "d3d11va";
			accel2 = "d3d11";
		} else if (tmp.contains("VAAPI"))
			accel = "vaapi";
	...
	var img = lime.app.Application.current.window.readPixels(new lime.math.Rectangle(FlxG.scaleMode.offset.x, FlxG.scaleMode.offset.y,
